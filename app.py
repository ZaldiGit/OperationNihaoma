...
def render_ai_assistant_tab(tracking: pd.DataFrame) -> None:
    ...
    model = st.secrets.get("OPENAI_MODEL", os.getenv("OPENAI_MODEL", "gpt-5.4-mini"))
    ...
    if not api_key:
        ...
        st.code("OPENAI_API_KEY = \"isi_api_key_anda\"\nOPENAI_MODEL = \"gpt-5.4-mini\"", language="toml")
        ...
        with st.chat_message("assistant"):
            with st.spinner("Asisten sedang membaca data tracking..."):
                try:
                    ...
                    answer = call_openai_chat(messages, model=model, api_key=api_key)
                    if not answer:
                        answer = "Maaf, respons AI kosong. Coba ulangi pertanyaan dengan lebih spesifik."
                except Exception as exc:
                    answer = f"Maaf, AI Assistant belum bisa menjawab. Detail error: {safe_text(exc)}"
                st.markdown(answer)
