...
def call_gemini_chat(messages: List[Dict[str, str]], model: str, api_key: str) -> str:
    ...
    payload: Dict[str, Any] = {
        ...
        "generationConfig": {
            "temperature": 0.25,
            "maxOutputTokens": int(st.secrets.get("AI_MAX_OUTPUT_TOKENS", os.getenv("AI_MAX_OUTPUT_TOKENS", "4096"))),
        },
    }
...
def _tracking_days_to_deadline(value: Any) -> Any:
    deadline = pd.to_datetime(value, errors="coerce")
    if pd.isna(deadline):
        return None
    today = pd.Timestamp(now_jakarta().date())
    return int((deadline.normalize() - today).days)


def _tracking_priority_score(value: Any) -> int:
    text = safe_text(value).strip().lower()
    mapping = {"urgent": 4, "tinggi": 3, "sedang": 2, "rendah": 1}
    return mapping.get(text, 0)


def _tracking_followup_rank(row: Dict[str, Any]) -> float:
    progress = normalize_tracking_progress(row.get("progress_percent") or row.get("progress_score"))
    stage = safe_text(row.get("stage") or tracking_stage(row)).strip().lower()
    priority_score = _tracking_priority_score(row.get("prioritas"))
    next_action_empty = 1 if not safe_text(row.get("next_action")).strip() else 0
    days = _tracking_days_to_deadline(row.get("deadline"))

    score = 0.0
    score += priority_score * 20
    score += max(0, 60 - progress) * 0.8

    if stage in ["belum mulai", "on progress", "waiting review"]:
        score += 18
    elif stage in ["submitted", "interview"]:
        score += 10
    elif stage in ["rejected", "revoked", "withdraw"]:
        score += 8
    ...
    if next_action_empty:
        score += 12

    return score


def _prepare_ai_local_df(tracking: pd.DataFrame) -> pd.DataFrame:
    df = prepare_tracking_df(tracking).copy()
    if df.empty:
        return df

    for col in ["nama_siswa", "program", "universitas", "jurusan", "stage", "prioritas", "deadline", "pic", "next_action", "catatan"]:
        if col not in df.columns:
            df[col] = ""

    df["progress_percent"] = df["progress_percent"].apply(normalize_tracking_progress)
    df["days_to_deadline"] = df["deadline"].apply(_tracking_days_to_deadline)
    df["rank_score"] = df.apply(lambda r: _tracking_followup_rank(r.to_dict()), axis=1)
    return df


def _format_deadline_for_ai(value: Any, days: Any) -> str:
    date_text = format_date_id(value) if safe_text(value) else "-"
    if days is None:
        return date_text
    if days < 0:
        return f"{date_text} (lewat {abs(days)} hari)"
    if days == 0:
        return f"{date_text} (hari ini)"
    return f"{date_text} ({days} hari lagi)"


def _tracking_reason(row: Dict[str, Any]) -> str:
    reasons = []
    progress = normalize_tracking_progress(row.get("progress_percent"))
    stage = safe_text(row.get("stage")).strip()
    priority = safe_text(row.get("prioritas")).strip()
    next_action = safe_text(row.get("next_action")).strip()
    days = row.get("days_to_deadline")

    if priority.lower() in ["urgent", "tinggi"]:
        reasons.append(f"prioritas {priority}")
    if progress <= 20:
        reasons.append(f"progress masih rendah ({progress:.0f}%)")
    elif progress <= 40:
        reasons.append(f"progress belum tinggi ({progress:.0f}%)")
    if stage.lower() in ["belum mulai", "on progress", "waiting review"]:
        reasons.append(f"status masih {stage}")
    if days is not None:
        if days < 0:
            reasons.append("deadline sudah lewat")
        elif days <= 3:
            reasons.append("deadline sangat dekat")
        elif days <= 7:
            reasons.append("deadline dekat")
    if not next_action:
        reasons.append("next action belum diisi")

    return "; ".join(reasons) if reasons else "perlu monitoring berkala"


def local_ai_followup_answer(tracking: pd.DataFrame, mode: str = "followup") -> str:
    df = _prepare_ai_local_df(tracking)
    if df.empty:
        return "Belum ada data student tracking yang bisa dianalisis."

    terminal = ["done", "ready to depart"]
    active = df[~df["stage"].astype(str).str.lower().isin(terminal)].copy()
    if active.empty:
        return "Semua data yang tersedia sudah berada pada status akhir. Tidak ada follow-up aktif yang mendesak."

    if mode == "mandek":
        candidate = active[
            (active["progress_percent"] <= 40)
            | (active["stage"].astype(str).str.lower().isin(["belum mulai", "on progress", "waiting review"]))
        ].copy()
        title = "Daftar mahasiswa dengan progress mandek atau rendah"
    else:
        candidate = active.copy()
        title = "Daftar prioritas follow-up hari ini"

    if candidate.empty:
        candidate = active.copy()

    candidate = candidate.sort_values(["rank_score", "progress_percent"], ascending=[False, True]).head(10)

    lines = [f"### {title}", ""]
    lines.append("Berikut urutan prioritas berdasarkan progress, status, deadline, prioritas, dan ketersediaan next action:")
    lines.append("")

    for idx, (_, row) in enumerate(candidate.iterrows(), start=1):
        rowd = row.to_dict()
        nama = safe_text(rowd.get("nama_siswa")) or "-"
        univ = safe_text(rowd.get("universitas")) or "-"
        program = safe_text(rowd.get("program")) or "-"
        stage = safe_text(rowd.get("stage")) or "-"
        progress = normalize_tracking_progress(rowd.get("progress_percent"))
        prioritas = safe_text(rowd.get("prioritas")) or "-"
        deadline = _format_deadline_for_ai(rowd.get("deadline"), rowd.get("days_to_deadline"))
        pic = safe_text(rowd.get("pic")) or "-"
        next_action = safe_text(rowd.get("next_action")) or "Belum ada next action"
        reason = _tracking_reason(rowd)

        lines.extend([
            f"**{idx}. {nama} — {univ} — {program}**",
            f"- Status/progress: **{stage}**, **{progress:.0f}%**",
            f"- Prioritas/deadline/PIC: **{prioritas}** • {deadline} • {pic}",
            f"- Alasan: {reason}",
            f"- Tindak lanjut: {next_action}",
            "",
        ])

    lines.append("**Saran kerja hari ini:** mulai dari 3 nama teratas, update `next_action`, lalu catat hasil follow-up di kolom catatan agar history progress tetap rapi.")
    return "\n".join(lines).strip()


def local_ai_dashboard_summary(tracking: pd.DataFrame) -> str:
    df = _prepare_ai_local_df(tracking)
    if df.empty:
        return "Belum ada data student tracking untuk diringkas."

    stage_counts = df.groupby("stage", dropna=False).size().sort_values(ascending=False)
    avg_progress = df["progress_percent"].mean()
    total = len(df)
    submitted = int(df["sudah_submit"].astype(str).str.lower().isin(["yes", "re-submit", "submitted"]).sum()) if "sudah_submit" in df.columns else 0
    loa = int(df["loa"].astype(str).str.lower().isin(["issued", "yes", "received"]).sum()) if "loa" in df.columns else 0
    low = int((df["progress_percent"] <= 30).sum())

    lines = [
        "### Ringkasan student tracking",
        "",
        f"- Total aplikasi: **{total}**",
        f"- Rata-rata progress: **{avg_progress:.0f}%**",
        f"- Sudah submit: **{submitted}**",
        f"- LOA issued: **{loa}**",
        f"- Progress rendah atau mandek (≤30%): **{low}**",
        "",
        "**Distribusi status:**",
    ]
    for stage, count in stage_counts.items():
        lines.append(f"- {safe_text(stage) or 'Belum Diisi'}: **{int(count)}**")

    top_follow = df.sort_values(["rank_score", "progress_percent"], ascending=[False, True]).head(3)
    if not top_follow.empty:
        lines.append("")
        lines.append("**3 prioritas follow-up teratas:**")
        for idx, (_, row) in enumerate(top_follow.iterrows(), start=1):
            lines.append(
                f"{idx}. **{safe_text(row.get('nama_siswa')) or '-'}** — "
                f"{safe_text(row.get('universitas')) or '-'} — "
                f"{safe_text(row.get('stage')) or '-'} ({normalize_tracking_progress(row.get('progress_percent')):.0f}%)"
            )

    return "\n".join(lines).strip()


def maybe_answer_locally_from_tracking(tracking: pd.DataFrame, user_prompt: str) -> str:
    prompt = safe_text(user_prompt).lower()
    if any(key in prompt for key in ["progress mandek", "progress rendah", "mandek atau rendah"]):
        return local_ai_followup_answer(tracking, mode="mandek")
    if any(key in prompt for key in ["follow-up", "follow up", "difollow-up", "prioritas tindak lanjut", "perlu difollow"]):
        return local_ai_followup_answer(tracking, mode="followup")
    if any(key in prompt for key in ["ringkasan dashboard", "ringkas progress", "ringkasan kondisi", "laporan singkat"]):
        return local_ai_dashboard_summary(tracking)
    return ""
...
7. Jawab dengan format Markdown yang selesai, bukan potongan heading. Batasi daftar maksimal 10 mahasiswa agar tidak terpotong.
8. Jangan menulis heading tanpa isi. Setiap nama harus punya alasan dan tindak lanjut singkat.
""".strip()
    ...
    if user_prompt:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Asisten sedang membaca data tracking..."):
                try:
                    # Untuk tombol analisis data rutin, jawab langsung dari dataframe agar hasil lengkap,
                    # cepat, dan tidak terpotong oleh batas token model AI.
                    answer = maybe_answer_locally_from_tracking(tracking, user_prompt)
