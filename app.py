...
def dominant_tracking_stage(series: pd.Series) -> str:
    cleaned = series.astype(str).str.strip().replace("", "Belum Mulai")
    if cleaned.empty:
        return "Belum Mulai"
    return safe_text(cleaned.value_counts().idxmax()) or "Belum Mulai"
...
def render_student_tracking_module(students_df: pd.DataFrame, tracking_df: pd.DataFrame, refs: Dict[str, Any]) -> None:
    ...
    with tabs[0]:
        if tracking.empty:
            ...
            with right:
                uni_base = tracking.copy()
                uni_base["universitas"] = uni_base["universitas"].replace("", "Belum Diisi")
                uni_df = (
                    uni_base.groupby("universitas", dropna=False)
                    .agg(
                        progress_percent=("progress_percent", "mean"),
                        stage=("stage", dominant_tracking_stage),
                    )
                    .reset_index()
                ...
                fig_uni.update_traces(
                    texttemplate="%{y:.0f}%",
                    hovertemplate="<b>%{x}</b><br>Rata-rata progress: %{y:.0f}%<br>Status dominan: %{customdata[0]}<extra></extra>",
                )
