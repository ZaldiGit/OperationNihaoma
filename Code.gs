...
function getRefColumn_(ss, headerName) {
  ...
  for (let i = 1; i < values.length; i++) {
    const v = values[i][colIndex];
    if (String(v || '').trim() !== '') out.push(String(v).trim());
  }
  return out;
}
    references: {
  	status_proses: getRefColumn_(ss, 'status_proses_list'),
  	status_verifikasi: getRefColumn_(ss, 'status_verifikasi_list'),
  	status_pelunasan: getRefColumn_(ss, 'status_pelunasan_list'),
  	status_pengiriman: getRefColumn_(ss, 'status_pengiriman_list'),
  	prioritas: getRefColumn_(ss, 'prioritas_list'),
  	next_action: getRefColumn_(ss, 'next_action_list'),
  	required_doc_types: getRefColumn_(ss, 'doc_type_list'),
  	metode_pembayaran: getRefColumn_(ss, 'payment_method_list'),
},
    meta: {
